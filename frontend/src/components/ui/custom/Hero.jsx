...
function Hero() {
  ...
  return (
    <div className="relative w-full min-h-[calc(100vh-64px)] overflow-hidden border-y border-white/10 bg-transparent px-5 py-12 sm:px-8 lg:px-12">
      <Suspense fallback={null}>
        <GlobeCanvas className="absolute inset-0 opacity-[0.98]" particleCount={1800} />
      </Suspense>
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_22%,rgba(56,189,248,0.09),transparent_42%),radial-gradient(circle_at_88%_18%,rgba(147,51,234,0.09),transparent_44%)]" />
      <div className="absolute inset-0 bg-gradient-to-b from-slate-950/25 via-slate-900/8 to-slate-950/45" />
          ...
          {[
            {
              ...
              description: "Optimizes sequence, travel gaps, and day pacing based on your preferences.",
            },
            {
              title: "Live Globe Visualization",
              description: "See travel arcs, orbital data rings, and destination context in immersive 3D.",
            },
            {
              title: "Weather-Aware Planning",
              description: "Shifts activities around rain and heat to protect comfort and time.",
            },
          ].map((card) => (
        ...
        >
          ...
          <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
            {destinations.map((place) => (
              <article
                key={place.name}
                className="group relative h-56 overflow-hidden rounded-2xl border border-white/20 bg-slate-900/25"
              >
                <img
                  src={place.image}
