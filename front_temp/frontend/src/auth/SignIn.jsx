...
export default function SignIn() {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data?.message || "Sign-up failed. Please try again.");
      navigate("/login");
    } catch (submitError) {
  ...
  const fields = [
    ...
    { name: "password", type: "password", label: "Password", placeholder: "Min. 6 characters", minLength: 6 },
    { name: "confirmPassword", type: "password", label: "Confirm password", placeholder: "Repeat password", minLength: 6 },
  ];
      ...
      >
        <div className="mb-7">
          ...
          <p className="mt-1.5" style={{ color: "rgba(237,231,218,0.5)", fontSize: "0.9rem" }}>
            Start building optimized trips in seconds
          </p>
        </div>
        ...
        <p className="text-sm text-center" style={{ color: "rgba(237,231,218,0.4)" }}>
          ...
          <Link to="/login" style={{ color: "var(--gold-300)" }} className="font-medium hover:underline">
            Log in
          </Link>
        </p>
      </div>
